...
| 变更       | 旧                            | 新                                                     |
| ---------- | ----------------------------- | ------------------------------------------------------ |
| 父状态     | 9 个线性状态                  | 5 个 PMBOK 状态：启动→计划→执行(含监控)→收尾→归档      |
| 状态层级   | 单字段 `ProjectStatus`        | 两级：父状态 + 可配置子状态/里程碑                     |
| 守卫逻辑   | `GuardContext` 7 个硬编码字段 | 数据/模板驱动                                          |
| 子状态配置 | 无                            | 运营后台配模板，不同项目类型不同模板                   |
| 里程碑联动 | 无                            | 子状态 = 任务树中的 milestone 任务，完成触发父状态流转 |
| `已中止`   | 独立状态                      | 特殊标记，任一父状态内可进入，需填写原因               |

### 任务管理

- 10 状态不变，最多 3 层子任务
- 依赖关系：FS/SS/FF/SF × 内/外 × 强/弱，模板生成 + 手动编排
- 里程碑与项目子状态同属一套数据体系

### 标准管理 — 三阶段上线

- **Phase A**: 任务模板 CRUD（后端 API + 运营 UI）
- **Phase B**: 项目模板组合（选任务模板 → 组合成项目模板）
- **Phase C**: 拖拽式流程编辑器
- 现状：类型系统 + 实例化引擎完备，数据全端硬编码

### 采购管理 — 三模块

- 供应商管理（已有 MVP，需后端化）
- 商品类目管理（树形电商结构，与供应商多对多）
- 项目采购管理（项目计划阶段分解采购计划）

### 跨域架构决策

- **快照迁移**: 删除全部快照相关代码（后端端点、serverAdapter、repository 引用）
- **后端框架**: 本次迭代迁回 Express，拆分为 routes/controllers/services
