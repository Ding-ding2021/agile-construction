...
### 7.1 ui-layout-rules — UI 布局规范

| 项目         | 内容                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------- |
| **说明**     | 2px 跳跃间距、禁止纯黑纯白、表格灰色边框、组件复用优先、布局骨架规范。所有 UI 评估/验收/开发者必须参照。 |
| **触发条件** | 涉及前端样式开发、UI 评估、UI 验收时。                                                                   |
| **关键词**   | 间距规范、色彩规范、组件复用、布局规范                                                                   |

---

## 8. Squad 小组协作

> 2 个子技能，实现开发前评估组 + 开发后验收组的双小组评审机制

### 7.1 squad-pre-dev-evaluation — 开发前评估

| 项目         | 内容                                                                                |
| ------------ | ----------------------------------------------------------------------------------- |
| **说明**     | 编码之前，派发评估小组从产品、UI/UX、技术三个视角并行评估需求，全票通过后才可开工。 |
| **触发条件** | 任何新需求/功能/变更在编码之前。L1 低风险任务可豁免。                               |
| **关键词**   | 评估需求、方案评审、多视角评估                                                      |

### 7.2 squad-post-dev-review — 开发后验收

| 项目         | 内容                                                                                                |
| ------------ | --------------------------------------------------------------------------------------------------- |
| **说明**     | 开发完成后，派发验收小组从功能、代码质量、UI 三个视角并行验收，全票通过后才可交付。打回后增量重审。 |
| **触发条件** | 开发交付者标记"完成待验收"时。L1 低风险任务可豁免。                                                 |
| **关键词**   | 验收成果、质量检查、代码评审                                                                        |
...
| "评估这个需求" / "看看这个方案"     | squad-pre-dev-evaluation                      |
| "检查UI规范" / "看布局合规"         | ui-layout-rules                               |
| "验收一下" / "检查完成质量"         | squad-post-dev-review                         |
