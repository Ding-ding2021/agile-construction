...
superseded_by: docs/01-product/product-plan-v1.2.md
related_code:
  - src-next/
  - local-api/
related_docs:
  - docs/01-product/product-planning-v1.md（L1 当前工作基线）
...
**V1（当前）**：围绕`三大核心能力`构建品牌方营建管理系统的 MVP 验证：

1. **原子化项目管理** — 将营建项目拆解为可独立管理的原子单元（任务），灵活适应不同品牌、不同业态的业务差异和资源方协作模式
2. **标准驱动执行** — 将品牌管理规范转化为可执行的数字标准库，驱动任务自动生成、执行指导与质量验收，实现「标准化管理」这一连锁经营的核心诉求
3. **AI 赋能降本** — 引入 AI Agent 降低专业门槛，让非专业人员也能高效管理建店项目，从模式上实现比传统营建管理更低的成本和更短的周期
...
> V1.1 / V2.3 已归档至 `docs/99-archive/`，以下为当前 V1.2 相对旧版本的核心调整：

| 变更项   | 旧版问题                                               | V1.2 处理方案                                                    |
| -------- | ------------------------------------------------------ | ---------------------------------------------------------------- |
| 战略叙事 | "内部标准化管理系统"口径窄化，未体现原子化和 AI 差异化 | 明确三大核心能力：原子化项目管理 + 标准驱动执行 + AI 赋能降本    |
| 技术路线 | 原定 Express + Prisma + PostgreSQL，但后端尚未启动     | 重新评估为 Node.js + Express + SQLite 轻量方案，V2 迁 PostgreSQL |
| 角色体系 | 旧版仅品牌方视角，V2 突然引入大量新角色                | 分阶段定义：V1 角色（必备）+ V2 角色（扩展）                     |
| 范围估算 | 旧版时间/成本预估为初步假设，缺乏实际依据              | 标注为"⏳ 待校准"，需基于 WBS 细化后再更新                       |
| 商业测算 | 旧版首年 2,379 万收入假设过于乐观                      | 保留在"战略展望"章节，标注假设前提                               |
...
## 1.2 V1 核心目标（验证 3 大核心能力）

1. **原子化项目管理** — 任务树模型是否能统一支撑项目、流程、服务三种业务形态？系统是否能灵活适配不同品牌、不同业态的管理差异？
2. **标准驱动执行** — 标准库是否能驱动任务生成、执行指导和验收判断？标准化管理是否能真正提升连锁建店的执行一致性和质量可控性？
3. **AI 赋能降本** — Agent 是否能在受控流程节点中降低专业门槛？非专业人员是否能在 AI 辅助下完成建店管理核心工作？

## 1.3 V1 核心闭环

> **项目是容器，任务是核心。项目的推进速度取决于任务的执行速度。**

```
需求接入 → 项目立项 → 任务模板匹配 → 任务树生成 → 任务独立推进(执行/采购/验收/整改) → 任务聚合完成 → 资产归档 → 结算建议
```

关键变化：项目不控制任务进度，任务是独立的执行单元，各自按自身状态机推进。每个任务的生命周期（草稿→待分配→执行中→待验收→已完成）独立管理，项目的整体进展从所有任务的聚合状态推导而来。
...
**核心原则：项目没有独立的状态机。项目状态从任务聚合推导而来。**

项目是一个容器，其"状态"不是硬编码的 9 个阶段，而是由其下属任务的进度聚合计算得出的。体现为：

> **每一分钟都在问：这个项目所有的任务，现在整体走到什么程度了？**

具体推导逻辑：

- **项目未启动** ↔ 所有任务均为`草稿`或`待分配`状态
- **项目执行中** ↔ 存在`执行中`、`待验收`状态的任务
- **项目待验收** ↔ 所有执行类任务已完成，但存在`待验收`的任务
- **项目整改中** ↔ 存在验收结果为`不通过`的任务
- **项目可归档** ↔ 所有任务均为`已完成`或`已关闭`的终态

> 项目级不设置人工推进的"状态按钮"。项目的推进速度取决于任务的执行速度。

**标签与任务状态的关系**：

- **领域标签**：回答"在这个领域我需要管理什么"——信息按 PMBOK 知识领域分类组织
- **任务状态**：回答"每个具体事情做到哪了"——每个任务有自己的状态机独立推进
- **项目聚合状态**：从任务状态汇总而来——不是硬编码的，是动态计算的
...
| **UI 组件**  | **shadcn/ui + Tailwind CSS v4**           | **已迁移至 `src-next/`，取代旧版 MUI 方案**    |
| **路由**     | Hash 路由（`#/projects/:code/:tab`）      | 保留，V2 再考虑 BrowserRouter                  |
| **图表**     | Recharts + 自定义组件（甘特/树形场景）    | 保留 Recharts，移除 MUI X 依赖                 |
...
| 任务状态机为 mock          | 系统停留在演示层面                | 接入真实后端后真实化            | 与后端同步 |
| 项目详情标签为生命周期阶段 | 与 PMBOK 知识领域不匹配，功能割裂 | 重构为 PMBOK 领域标签（8 标签） | 5-7 天     |

> ⚠️ **以下 5.7 节内容为旧方案记录，实际项目已迁移至 shadcn/ui + Tailwind CSS v4（`src-next/`）。保留此节作为历史参考，不再执行。**
...
| **P1** | 步骤条    | 任务聚合进度可视化、立项向导 | `Stepper`                             | 1 天       |
...
- `任务原子化`：任务是最小执行单元和状态单元。项目没有独立状态机，项目状态从任务聚合推导
- `结果对象独立`：任务推进动作，结果对象沉淀成果
- `任务状态驱动项目`：每个任务有独立状态机独立推进，项目进度取决于任务的执行速度。不设人工推进的"项目状态按钮"
- `人工兜底`：关键节点必须支持人工接管
...
> ⏳ **待校准**：以下实施路线图为 V1.2 规划草案，时间预估和交付安排尚未基于实际开发速度校准。当前实际代码已覆盖部分 Phase 1-2 内容（SQLite 后端、CSS 变量统一、通用组件），需重新盘点实际进度后更新排期。
...
| W3-W4 | 任务模型 + 本地后端           | SQLite 表结构（含任务表、任务关系表）、五接口本地联调 | 任务 CRUD 接口可本地读写                                |
| W5-W6 | 任务状态机实现 + 项目聚合逻辑 | 任务独立状态流转、项目状态从任务聚合推导              | 任务状态变更持久化到 SQLite；项目概览状态为实时聚合计算 |
...
| **技术风险** | shadcn/ui + Tailwind 主题定制在实际业务场景中视觉/交互不达预期 | 中     | 中       | 🟡 中等     | Phase 1 前两周完成关键页面（项目列表/项目详情）的视觉验证，不通过则回退到备选 UI 方案                                                                                         | 产品经理               |
| **技术风险** | SQLite 在并发场景（多用户同时操作）下性能瓶颈                  | 中     | 中       | 🟡 中等     | V1 试点期控制并发用户数（< 20人）；监控慢查询，若出现则提前启动 PostgreSQL 迁移                                                                                               | 后端负责人             |
| **技术风险** | 任务状态机逻辑复杂，AI 编码导致状态流转漏洞                    | 中     | 高       | 🟡 中等     | 任务状态机定义（`taskStatusMachine.ts`）人工维护，AI 仅做 UI 层实现；聚合推导逻辑人工编码；每个状态流转必须编写单元测试                                                       | 技术负责人             |
...
1. **任务状态机出现漏洞**：任务状态可非法跳转，或任务→项目聚合计算逻辑出现数据不一致
...
| 任务状态机 AI 编码漏洞       | 🟡 中等  | MVP 核心   | 任务状态流转单元测试覆盖率、非法跳转事件数 |
...
> ⏳ **待校准**：以下成本数字为 V1.2 规划阶段的假设性测算（AI Token 单价、人力成本、迭代轮次均为拍脑袋估算），尚未基于实际 Token 消耗和迭代耗时校准。保留作为初始假设，待完成 1-2 个模块开发后重新测算。
...
> ⏳ **待校准**：以下 ROI 数字为假设性测算，无实际数据支撑。150 万建设投入、5 人效率提升等关键假设均未经验证。
...
> 项目没有独立状态机。以下标签为任务聚合结果的观察口径，非可人工操作的"项目状态按钮"。

| 项目聚合标签 | 触发条件（任务聚合推导）                    |
| ------------ | ------------------------------------------- |
| 待立项       | 项目刚创建，尚未生成任务模板                |
| 待执行       | 任务已生成，所有任务均为 `待分配` 或 `草稿` |
| 执行中       | 存在 `执行中`、`待验收` 状态的任务          |
| 待验收       | 所有执行类任务已完成，存在 `待验收` 的任务  |
| 整改中       | 存在验收结果为 `不通过` 的任务              |
| 可归档       | 所有任务均为 `已完成` 或 `已关闭` 的终态    |

### 任务状态（V1.2 精简版）

- 待分配 → 待执行 → 执行中 → 待验收 → 已完成 → 已关闭

> 任务有自己的独立状态机。项目的推进速度取决于任务的执行速度。
...
| 版本 | 日期       | 变更内容                                                                                                                                                                                                                                                                                                  | 作者            |
| ---- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| V1.3 | 2026-05-13 | 修正 frontmatter 标题为"数字营建项目"；对齐技术栈正文与 shadcn/ui 实际情况；标注第 7-9 章数字为"待校准"（含实施排期、MVP 成本、ROI、V2 战略测算）                                                                                                                                                         | docs-maintainer |
