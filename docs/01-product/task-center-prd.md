...
- `task_type`
- `node_level_type`：project / work_package / task / subtask
- `business_domain`
...
- `work_package_id` 1:N `task_id`（含 subtask）
...
- 项目节点用于承接顶层目标，不直接执行
- 工作包用于表达交付范围与责任边界
- 任务用于表达最小可分配、可执行、可验收动作
- 子任务用于表达任务内部协同步骤，不作为独立成本归集层
- `阶段`不作为任务树层级节点，统一作为项目生命周期属性（如 `stage_id`）参与筛选、统计与权限分域
...
| 已完成 | 🟢 绿色  | 验收通过，任务闭环            | 系统自动              |
| 不通过 | 🔴 红色  | 验收未通过，需整改            | 验收/质检角色         |
| 已关闭 | ⚪ 灰色  | 任务被取消或关闭              | 平台项目经理          |
...
| `node_level_type`         | enum(`project`,`work_package`,`task`,`subtask`) | 结构关系 | 必填                                                     | 节点层级类型（项目/工作包/任务/子任务）       |
...
| 列名              | 说明                       | 示例                                                   |
| ----------------- | -------------------------- | ------------------------------------------------------ |
| `默认状态/默认值` | 字段在未手动赋值时的初始值 | `status=待创建`、`is_blocked=false`、`comment_count=0` |
...
| `status`           | `待创建`                  | 系统+人工    | 初始系统设定，后续按状态机由操作触发 |
