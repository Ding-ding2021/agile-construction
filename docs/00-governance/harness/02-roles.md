...
**1.2 [强制]** 角色之间严格分工，不得越界。产品经理不得代替设计师做 UI 决策，设计师不得代替开发者做后端实现，开发者不得代替测试写测试代码。
...
**2.2.1 [强制]** 我代表用户体验和视觉一致性，对人机交互品质负责。**前端代码实现也是我的职责**。
...
| 2.4.2.2 | 测试框架维护 | **维护 Vitest/Playwright 等测试基础设施**            |
| 2.4.2.3 | 自动化测试   | **编写和执行单元测试、组件测试、API 测试、E2E 测试** |
| 2.4.2.4 | 缺陷跟踪     | 发现、记录、跟踪缺陷                                 |
| 2.4.2.5 | 质量门禁     | 把控质量标准，不符合条件的禁止上线                   |
