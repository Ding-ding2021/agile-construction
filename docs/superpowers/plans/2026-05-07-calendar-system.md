...
// 创建默认日历
const calResult = db.prepare(`INSERT INTO calendars (name, description, is_default) VALUES (?, ?, ?)`).run(
  '中国建筑通用日历',
...
export function getWorkingDays(start: string, end: string, exceptions: DayStatus[]): string[] {
  ...
  return days.filter(d => isWorkingDay(d.toISOString().slice(0, 10), exceptions))
...
useEffect(() => {
  ...
  loadNonWorkingDays(from, to).then(days => {
...
const nonWorkingCols = useMemo(() => {
  ...
  return days
    .map((d, i) => {
      const dateStr = d.toISOString().slice(0, 10)
      return { index: i, isNonWorking: nonWorkingDaySet.has(dateStr) || d.getDay() === 0 || d.getDay() === 6 }
    })
