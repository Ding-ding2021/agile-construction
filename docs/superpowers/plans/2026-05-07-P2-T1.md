...
describe('buildGanttRows', () => {
  it('扁平节点转为甘特行', () => {
    const nodes = [
      { id: 1, plannedStart: '2026-05-10', duration: 5 },
    ] as WBSNode[]
...
export function WBSGanttBar({ node, dayStart, daySpan, dayWidth, depth, isCritical, onClick }: WBSGanttBarProps) {
        ...
        <div className="overflow-x-auto">
          <div style={{ minWidth: totalDays * DAY_WIDTH + 200 }}>
            ...
            <div className="relative" style={{ minHeight: rows.length * 28 }}>
              {rows.map(row => (
                <div key={row.node.id} className="relative h-7" style={{ marginLeft: row.depth * 16 }}>
                  <Tooltip>
...
export function WBSGanttDependencyArrows({ barPositions }: { barPositions: Map<number, { x: number; y: number }> }) {
  const flatNodes = useWBSStore(s => s.flatNodes)
...
export const WBSNetworkNode = memo(function WBSNetworkNode({ data, selected }: NodeProps<WBSNetworkNodeData>) {
...
export function WBSNetworkDiagram() {
  ...
  const onNodeClick = useCallback((_event: React.MouseEvent, node: Node<WBSNetworkNodeData>) => {
  ...
  return (
    <div className="flex-1">
      ...
      >
        ...
        <Controls />
        <MiniMap
          nodeColor={n => n.data?.isCritical ? 'hsl(var(--chart-1))' : 'hsl(var(--muted-foreground))'}
