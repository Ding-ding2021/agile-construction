...
function handleTasks(req: IncomingMessage, res: ServerResponse, projectCode: string): void {
  ...
  if (req.method === 'GET') {
    const rows = db
      .prepare(
        `
      SELECT id, project_id as projectId, code, name, status, assignee,
             start_date as startDate, due_date as dueDate, progress, level, parent_id as parentId
      FROM project_tasks WHERE project_id = ? ORDER BY id ASC
    `
      )
      .all(project.id)
    sendJson(res, rows)
  ...
  if (req.method === 'POST') {
    parseBody<Record<string, unknown>>(req)
      .then(body => {
        const stmt = db.prepare(`
        INSERT INTO project_tasks (project_id, code, name, status, assignee, start_date, due_date, progress, level, parent_id)
        VALUES (@projectId, @code, @name, @status, @assignee, @startDate, @dueDate, @progress, @level, @parentId)
      `)
        const result = stmt.run({
...
function handleTaskById(
  ...
  if (req.method === 'PUT') {
    parseBody<Record<string, unknown>>(req)
      .then(body => {
        const { id: _, projectId: __, ...data } = body
        const setClauses: string[] = []
        const params: Record<string, unknown> = {}
        for (const [k, v] of Object.entries(data)) {
          const col = k.replace(/[A-Z]/g, m => '_' + m.toLowerCase())
