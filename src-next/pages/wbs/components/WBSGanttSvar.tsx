...
const ZOOM_PRESETS = [
  { label: '日', cellWidth: 40, scales: [
    { unit: 'month' as const, step: 1, format: '%Y年%m月' },
    { unit: 'day' as const, step: 1, format: '%j', css: (d: Date) => d.getDay() === 0 ? 'svar-day-nonworking' : '' },
  ]},
  { label: '周', cellWidth: 20, scales: [
    { unit: 'month' as const, step: 1, format: '%m月' },
    { unit: 'week' as const, step: 1, format: 'W%w' },
  ]},
  { label: '月', cellWidth: 8, scales: [
    { unit: 'year' as const, step: 1, format: '%Y年' },
    { unit: 'month' as const, step: 1, format: '%m月' },
  ]},
  { label: '年', cellWidth: 2, scales: [
    { unit: 'year' as const, step: 1, format: '%Y年' },
...
export function WBSGanttSvar() {
  ...
  const [svarApi, setSvarApi] = useState<{ exec: (a: string, d?: unknown) => void } | null>(null)

  const p = ZOOM_PRESETS[preset]

  const tasks = useMemo(() => {
    const parentIds = new Set(flatNodes.map(n => n.parentId).filter(Boolean))
    const cpmResult = calculateCriticalPath(flatNodes.map(n => ({
      id: n.id, duration: n.duration || 1,
    ...
    return flatNodes.map(n => ({
      ...
      duration: n.duration || 1, progress: (n.progress || 0) / 100,
      parent: n.parentId || 0,
      type: parentIds.has(n.id) ? ('summary' as const) : ('task' as const),
      status: n.status, isCritical: cpmResult.get(n.id)?.isCritical || false,
    }))
  }, [flatNodes])
  ...
  const links = useMemo(() => {
    ...
    for (const n of flatNodes) {
      ...
      for (const depId of deps) {
        if (ids.has(depId)) result.push({ id: `${depId}-${n.id}`, source: depId, target: n.id, type: 'e2s' as const })
      }
    }
    return result.length > 0 ? result : undefined
  }, [flatNodes])

  const handleUpdateTask = useCallback((ev: { id: number; start?: string; end?: string; duration?: number; progress?: number }) => {
    const id = Number(ev.id); if (!id) return
  ...
  if (flatNodes.length === 0) {
    return <div className="flex-1 flex items-center justify-center text-muted-foreground text-sm">暂无 WBS 节点</div>
  }
        ...
        {ZOOM_PRESETS.map((zp, i) => (
          <button key={zp.label}
            className={cn('rounded px-2 py-0.5 text-[11px] font-medium transition-colors',
              i === preset ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}
            onClick={() => setPreset(i)}
          >{zp.label}</button>
        ))}
