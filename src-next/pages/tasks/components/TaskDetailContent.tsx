...
import TaskChecklist from './info/TaskChecklist'
...
export default function TaskDetailContent({
  ...
  const handleStatusChange = useCallback((_code: string, status: TaskStatus) => {
    if (!pc || !taskId) return
  ...
  const handleRemind = useCallback((_code: string) => {
    if (!pc || !taskId) return
    api.remindTask(pc, taskId).then(() => {
      toast.success('已催办')
      onReload?.()
    }).catch(() => toast.error('催办失败'))
  }, [pc, taskId, onReload])

  const handleAssigneeSave = useCallback((_code: string, data: { assigneeName?: string; plannedStartAt?: string; plannedEndAt?: string }) => {
    if (!pc || !taskId) return
    api.updateTask(pc, taskId, data).then(() => {
      toast.success('已保存')
    }).catch(() => toast.error('保存失败'))
  }, [pc, taskId])

  const handleRiskChange = useCallback((_code: string, riskLevel: string) => {
    if (!pc || !taskId) return
    api.updateTask(pc, taskId, { riskLevel }).then(() => {
  ...
  const handleChecklistToggle = useCallback((itemId: string) => {
    ...
    if (!item) return
    api.toggleChecklist(pc, taskId, Number(itemId), !item.done)
      .then(() => onReload?.())
      .catch(() => toast.error('操作失败'))
  }, [pc, taskId, task.checklist, onReload])

  const handleAddRelation = useCallback((fromTaskId: string, relationType: string) => {
    if (!pc || !taskId) return
    api.addRelation(pc, taskId, fromTaskId, relationType)
      .then(() => { toast.success('前置任务已添加'); onReload?.() })
      .catch(e => toast.error(e.message))
  }, [pc, taskId, onReload])

  const handleRemoveRelation = useCallback((relationId: string) => {
    if (!pc || !taskId) return
    api.removeRelation(pc, taskId, Number(relationId))
      .then(() => { toast.success('前置任务已移除'); onReload?.() })
      .catch(() => toast.error('操作失败'))
  }, [pc, taskId, onReload])

  const handleTagsChange = useCallback((_code: string, tags: string[]) => {
    if (!pc || !taskId) return
  ...
  const handleProgressChange = useCallback((value: number) => {
    if (!pc || !taskId) return
    api.updateTask(pc, taskId, { progress: value })
  ...
  const header = (
    <TaskDetailHeader task={task} mode={mode} onBack={onBack} onPrev={onPrev} onNext={onNext} onOpenPage={onOpenPage} />
  )
    ...
    <>
      ...
      {leftTab === 'detail' ? (
        <>
          ...
          <TaskPeople task={task} readonly={readonly} onSave={handleAssigneeSave} assigneeOptions={assigneeOptions} />
          <Separator />
          <TaskChecklist items={task.checklist ?? []} readonly={readonly} onToggle={handleChecklistToggle} />
          ...
          <TaskAttachments attachments={task.attachments ?? []} readonly={readonly} onUpload={onUploadAttachment} />
        </>
      ) : (
        <>
          <Card className="p-4 space-y-3">
            <TaskStatusOps task={task} readonly={readonly} onStatusChange={handleStatusChange} onRemind={handleRemind} />
          ...
          />
          <TaskProgress progress={task.progress} readonly={readonly} onChange={handleProgressChange} />
