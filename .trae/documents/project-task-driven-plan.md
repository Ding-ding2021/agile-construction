...
## Pre-dev Squad 仲裁决策（2026-05-15 人类批准）

以下 9 项条件已由老板批准，熔入本计划：

| #   | 决策                                                                                                                          | 来源                  |
| --- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | `computeAcceptanceStatus` / `computeSettlementStatus` 接受结构化数据数组而非 projectId → 所有 7 个函数都是纯函数              | 开发+测试 联合建议    |
| 2   | parentStatus 推导规则：创建→"启动"，首次WBS→"计划"，首个任务执行→"执行"，首个检查项创建→"监控"，全部验收通过→"收尾"（不可逆） | 产品建议              |
| 3   | 在 parentStatus 中增加"中止"为终态（不可逆），处理项目暂停/取消场景                                                           | 产品建议              |
| 4   | 维度面板使用 shadcn `Progress` 水平条，不手写 SVG 环形进度条                                                                  | UI/UX 建议 + 老板选定 |
| 5   | 抽取 `afterTaskUpdate(taskId)` 钩子函数，统一管理快照生成 + 聚合触发两个副作用                                                | 开发建议              |
| 6   | 验收/采购模块的聚合触发拆分为独立子任务（1.7a: acceptance, 1.7b: procurement）                                                | 开发建议              |
| 7   | 健康度不独立成 tab，作为概览页的增强区域                                                                                      | UI/UX 建议 + 老板拍板 |
| 8   | `statusTone` 随旧 `status` 一同移除，健康度颜色从新聚合结果计算                                                               | 开发建议 + 老板批准   |
| 9   | `executionStatus` 规则：存在至少 1 个已完成任务 → "进行中"；全部终态 → "已完成"                                               | 测试建议 + 老板批准   |
